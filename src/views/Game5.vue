<template>
  <div class="game-container">
    <h1>Jeu de Lettres: {{ name }}</h1>

    <div class="instructions">
      <p>Tape les lettres du prénom "{{ name }}" dans l'ordre</p>
    </div>
    ...
    <div v-if="gameCompleted" class="victory-message">
      <h2>Bravo !</h2>
      <p>Tu as réussi à écrire {{ name }} !</p>
...
export default {
  ...
  created() {
    // Select a random name when the component is created
    this.selectRandomName();

    // Skip spaces at the beginning if any
    this.$nextTick(() => {
      this.skipSpaces();
    });
  },
  ...
  methods: {
    selectRandomName() {
      ...
      this.name = this.namesList[randomIndex];
    },
        ...
        if (pressedKey === expectedLetter) {
          ...
          setTimeout(() => {
            ...
            if (this.currentLetterIndex >= this.name.length) {
              ...
              // Skip spaces for the next letter
              this.skipSpaces();
            }
          }, 1);
        } else {
    ...
    skipSpaces() {
      // Skip spaces automatically until we find a non-space character or reach the end
      while (
        this.currentLetterIndex < this.name.length && 
        this.name[this.currentLetterIndex] === ' '
      ) {
        this.currentLetterIndex++;
      }
    },
